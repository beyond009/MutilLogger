...
shared(installer) actor class MultiLogger() = this{
    ...
    private type ManageActor = actor {
    ...
    private type Error = {
        #MsgInputSizeErr;    private type canister_id = Principal;
    ...
    private func _append(msgs : [Text]) : async Result.Result<Text, Error> {
    ...
    private func _view(from : Nat, to : Nat) : async Result.Result<[Text], Error> {
    ...
    private func _getNewLogger() : async () {
